...
def get_reviews(product_id, max_reviews=MAX_REVIEWS_PER_PRODUCT):
    ...
    for review_page in range(1, max_pages + 1):
        ...
        for review in review_list:
            ...
            reviews.append(
                {
                    "review_id": normalize_id(review.get("id") or review.get("review_id")),
...
def build_product_rows(product, keyword, max_reviews):
    ...
    reviews = []
    if review_count > 0:
        reviews = get_reviews(product_id, max_reviews=max_reviews)
...
def crawl_keyword(keyword, start_page, end_page, products_per_page, max_reviews):
    ...
    for page in tqdm(range(start_page, end_page + 1), desc=f"Crawling {keyword}"):
        ...
        for idx, product in enumerate(products, start=1):
            ...
            all_rows.extend(build_product_rows(product, keyword, max_reviews))
